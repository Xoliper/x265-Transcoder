...
int FrameEncoder::s_threadCounter = 0;
...
{	
    ...
    for (uint32_t cuAddr = 0; cuAddr < lastCUAddr; cuAddr++)
    {
        ...
        if (saoParam)
        {
			...
			if (m_param->tc != NULL) {
				m_param->tc->workOnSAO(ctu, saoParam, m_threadId, m_frame->m_poc, m_param->internalCsp);
			}
...
void FrameEncoder::processRowEncoder(int intRow, ThreadLocalData& tld)
{
    ...
    while (curRow.completed < numCols)
    {
		...
		Mode best;
		if (m_param->tc != NULL && m_param->tc->getTranscoderMode() == WRITE_CTU) {
			best = tld.analysis.compressCTU(*ctu, *m_frame, m_cuGeoms[m_ctuGeomMap[cuAddr]], rowCoder);
		}
		else if (m_param->tc == NULL) {
			best = tld.analysis.compressCTU(*ctu, *m_frame, m_cuGeoms[m_ctuGeomMap[cuAddr]], rowCoder);
		}

		if (m_param->tc != NULL ) {
			m_param->tc->workOnCTU(ctu, m_threadId, m_frame->m_poc, m_param->internalCsp);
		}


		if (ctu->m_slice->m_sliceType == B_SLICE && m_param->tc->getTranscoderMode() == READ_CTU) {
			m_param->tc->recursiveCU(ctu, *m_cuGeoms, 0);
		}
