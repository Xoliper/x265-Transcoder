...
#define OPT2(STR1, STR2) else if (!strcmp(name, STR1) || !strcmp(name, STR2))
	...
	OPT("throw-prop") p->throwProp = (uint16_t)atoi(value);
	OPT("ones-throw") p->throwOnes = (uint16_t)atoi(value);
	OPT("transcoder-mode") p->tcMode = (uint16_t)atoi(value);
	OPT("transcoder-file") {
		void* pTmp = malloc(sizeof(Transcoder));
		p->tc = new (pTmp) Transcoder(value);	
		//p->tc = new Transcoder(value);
	}
