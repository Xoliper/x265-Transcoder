...
{
    m_chromaFormat  = csp;
	m_hChromaShift = (csp == X265_CSP_I420 || csp == X265_CSP_I422);//CHROMA_H_SHIFT(csp);
    m_vChromaShift  = CHROMA_V_SHIFT(csp); //1 lub 0;
    m_numPartitions = NUM_4x4_PARTITIONS >> (depth * 2);
	m_dynamicDepth = depth;
...
void CUData::initCTU(const Frame& frame, uint32_t cuAddr, int qp)
{
	...
	m_cuBelow = (m_cuAddr / widthInCU < m_slice->m_sps->numCuInHeight - 1) ? m_encData->getPicCTU(m_cuAddr + widthInCU) : NULL;
	m_cuRight = (m_cuAddr % widthInCU < widthInCU - 1) ? m_encData->getPicCTU(m_cuAddr + 1) : NULL;
	m_cuBelowRight = (m_cuBelow && m_cuRight) ? m_encData->getPicCTU(m_cuAddr + widthInCU + 1) : NULL;
	m_cuBelowLeft = (m_cuLeft && m_cuBelow) ? m_encData->getPicCTU(m_cuAddr + widthInCU - 1) : NULL;
