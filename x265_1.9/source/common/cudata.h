...
public:
	...
	const CUData* m_cuBelow;      // pointer to above-left neighbor CTU
	const CUData* m_cuRight;     // pointer to above-right neighbor CTU
	const CUData* m_cuBelowLeft;          // pointer to above neighbor CTU
	const CUData* m_cuBelowRight;           // pointer to left neighbor CTU

	uint32_t m_dynamicDepth;
...
//	const CUData* getPUBelow(uint32_t& bPartUnitIdx, uint32_t curPartUnitIdx) const;
//	const CUData* getPURight(uint32_t& aPartUnitIdx, uint32_t curPartUnitIdx) const;
//	const CUData* getPUBelowRight(uint32_t& arPartUnitIdx, uint32_t curPartUnitIdx) const;
	...
	const CUData * neighborCTU(int offset);
	int offset(uint32_t & pos_neighbor);
	const CUData * getCuNeighbor(uint32_t absIdx, uint32_t & neighbor, uint32_t log2CUSize, int vert, int hor);
